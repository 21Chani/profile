import { useBreakpointQuery } from "@/modules/global/hooks/useBreakpointValue"
...
export function TerminalRow({ date, children, className, path = PROFILE_PATH, ...props }: TerminalRowProps) {
  const isNarrow = useBreakpointQuery({ base: true, sm: false })

  const handledPath = isNarrow
    ? path
        .split("/")
        .map((dir, index) => (dir === "~" || index > 3 ? dir : dir.slice(0, 2)))
        .join("/")
    : path
        ...
        {date && (
          <div className="h-6 p-3 py-1 bg-gray-600 gap-0.5 flex rounded-r-sm">
            <p className="text-sm leading-4">
              {!isNarrow && "at"} {format(date, "hh:mm:ss")}
            </p>
